...
from app.schemas import NotificationType, MessageChannel, Notification
...
class FFmpegStrmThumb(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            try:
                ...
                self._gen_strategy_max_count = 100
                self._gen_strategy_delay = 60
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [{
            "path": "/gen_thumb",
            "endpoint": self.api_gen_thumb,
            "methods": ["GET", "POST"],
            "summary": "生成strm视频缩略图",
            "description": "传入strm文件路径���调用FFmpeg截取视频缩略图。",
        }]
                ...
                'content': [
                    {
                        ...
                        'content': [
                            ...
                            },
                            {
                                ...
                                'content': [{
                                    'component': 'VSwitch',
                                    'props': {'model': 'overlay', 'label': '覆盖生成'}
                                }]
                            }
                        ]
                    },
                        ...
                        'content': [
                            {
                                ...
                                'content': [{
                                    ...
                                    'props': {'model': 'timeline', 'label': '截取时间', 'placeholder': '00:03:01'}
                                }]
                            },
                            {
                                'component': 'VCol',
                                ...
                                'content': [{
                                    'component': 'VTextField',
                                    'props': {'model': 'gen_strategy', 'label': '生成策略', 'placeholder': '100=60'}
                                }]
                            },
                            {
                                'component': 'VCol',
                    ...
                    },
                    {
                        ...
                        'content': [{
                            ...
                            'content': [{
                                'component': 'VTextarea',
                                'props': {'model': 'scan_paths', 'label': '定时扫描路径',
                                          'rows': 5, 'placeholder': '每一行一个目录'}
                            }]
                        }]
                    },
                    {
                        ...
                        'content': [{
                            ...
                            'content': [{
                                'component': 'VTextarea',
                                'props': {'model': 'exclude_paths', 'label': '定时扫描排除路径',
                                          'rows': 2, 'placeholder': '每一行一个目录'}
                            }]
                        }]
                    },
                    {
                        ...
                        'content': [{
                            ...
                            'content': [{
                                ...
                                'props': {
                                    ...
                                    'text': '提供API：/api/v1/plugin/FFmpegStrmThumb/gen_thumb?strm_path=xxx&apikey=xxx，可对单个strm文件生成缩略图。'
                                }
                            }]
                        }]
                    }
    ...
    def api_gen_thumb(self, strm_path: str, apikey: str, timeline: str = None,
                      overlay: bool = False) -> Dict[str, Any]:
        """
        API：根据传入的strm文件路径生成缩略图
        :param strm_path: strm文件绝对路径
        :param apikey: 系统API_TOKEN
        :param timeline: 截取时间点，默认使用插件配置
        :param overlay: 是否覆盖已存在的缩略图
        """
        if apikey != settings.API_TOKEN:
            raise HTTPException(status_code=403, detail="apikey错误")
        if not strm_path:
            return {"code": 1, "message": "strm_path不能为空"}
        file_path = Path(strm_path)
        if not file_path.exists():
            return {"code": 1, "message": f"文件不存在：{strm_path}"}
        if file_path.suffix.lower() != ".strm":
            return {"code": 1, "message": f"不是strm文件：{strm_path}"}
        thumb_path = file_path.with_name(file_path.stem + "-thumb.jpg")
        if thumb_path.exists() and not overlay:
            return {
                "code": 0,
                "message": "缩略图已存在",
        ...
        try:
            ...
            if not video_url:
                return {"code": 1, "message": "strm文件内容为空"}
            frames = timeline or self._timeline or "00:03:01"
            with ffmpeg_lock:
                ok = self.get_thumb(strm_path=video_url,
                                    image_path=str(thumb_path),
                                    frames=frames)
            if ok and thumb_path.exists():
                logger.info(f"[API]{file_path} 缩略图已生成：{thumb_path}")
                return {
                    "code": 0,
                    "message": "生成成功",
                    "thumb_path": str(thumb_path),
            ...
            return {"code": 1, "message": "ffmpeg执行失败，未生成缩略图"}
        except Exception as err:
            logger.error(f"[API]处理 {strm_path} 失败：{str(err)}")
            return {"code": 1, "message": f"处理异常：{str(err)}"}
    ...
    def gen_file_thumb(self, file_path: Path, is_overlay):
        ...
        with ffmpeg_lock:
            try:
                thumb_path = file_path.with_name(file_path.stem + "-thumb.jpg")
                if not is_overlay and thumb_path.exists():
    ...
    def get_thumb(self, strm_path: str, image_path: str, frames: str = None):
        ...
        if not strm_path or not image_path:
            return False
        cmd = 'ffmpeg -ss {frames} -i "{strm_path}" -vframes 1 -f image2 "{image_path}"'.format(
            strm_path=strm_path, frames=frames, image_path=image_path)
